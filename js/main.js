...
(function ($) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const lyricsContent = document.querySelector('.lyrics-box .lyrics-content');

    const playButtons = document.querySelectorAll('.jp-play');

    playButtons.forEach(button => {
      button.addEventListener('click', function () {
        const parentContainer = this.closest('.single_player_container');
        const lyrics = parentContainer.getAttribute('data-lyrics');

        if (lyrics) {
          lyricsContent.innerHTML = lyrics.replace(/\n/g, '<br>');
          lyricsBox.classList.add('active');
        } else {
          lyricsContent.innerHTML = '<em>No lyrics available.</em>';
          lyricsBox.classList.add('active');
        }
      });
    });
